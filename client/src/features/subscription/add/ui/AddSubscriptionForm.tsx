...
export function AddSubscriptionForm({
  ...
  const activePresetCategory =
    presetCatalog.find((cat) => cat.name === presetCategory) || null;
  const effectivePeriod =
    editingId && date === initialDate
      ? (initialPeriod ?? "1 month")
      : inferPeriod(date);
  const serviceOptions = (activePresetCategory?.services || []).map(
    (service) => ({
      value: service.name,
      label: service.name,
      color: service.color,
  ...
  useEffect(() => {
    ...
    if (
      dbCategory &&
      presetCatalog.some((cat) => cat.name === dbCategory.name)
    ) {
  ...
  const handleServiceSelect = (serviceName: string) => {
    ...
    const service = activePresetCategory?.services.find(
  ...
  const validate = () => {
    ...
    if (isSplit && !splitUsername.trim())
      newErrors.splitUsername = "Обязательное поле";
    if (isSplit) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const splitAmountValue =
        splitMode === "pct"
          ? Math.round((Number(price) * Number(splitAmount)) / 100)
          : Math.round(unconvert(Number(splitAmount)));
  ...
  return (
    <form className="add-form" onSubmit={handleSubmit} noValidate>
      ...
      <FormField
        ...
        error={errors.date}
      />
      <span className="add-form__hint">
        Повторять: {periodLabel(effectivePeriod)}
      </span>
      <div className="add-form__row">
      ...
      <div className="add-form__row">
        <div className="add-form__setrow">
          ...
          <div
            className={`add-form__switch ${isSplit ? "add-form__switch--on" : ""}`}
          ...
          <div className="add-form__row">
            <div className="add-form__label-row">
              <span className="add-form__label">
                {splitMode === "rub" ? "Сумма доли" : "Доля в процентах"}
              </span>
