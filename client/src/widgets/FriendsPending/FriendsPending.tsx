...
export function FriendsPending({
  ...
  if (splits.length === 0) {
    return (
      ...
      >
        ...
        <span>
          Включи «Разделить оплату» при добавлении подписки — друг появится
          здесь со своей долей
        </span>
      </div>
    );
  }
  ...
  return (
    <>
      <div
        className="friends-pending__header rise"
        style={{ animationDelay: "0.14s" }}
      >
        <h2 className="friends-pending__title">
          <i></i>Ждут перевода
        </h2>
        <span className="friends-pending__sum">
          {convert(total).toLocaleString("ru-RU", { useGrouping: false })}{" "}
          {currency}
        </span>
      </div>
      <div
        className="friends-pending__list rise"
            ...
            <div className="friends-pending__card" key={split.id}>
              ...
              <div className="friends-pending__info">
                ...
                <div className="friends-pending__sub">
                  {subscription
                    ? `${subscription.title} · доля до ${formatNextDate(subscription.next_payment_date)}`
                    : "Подписка удалена"}
                </div>
              </div>
